...
const VideoForm = () => {
	...
	const schemaModel = yup.object().shape({
		...
		videoURL: yup.string().required("This field has to be filled"),
	});
	...
	const onSubmit = handleSubmit(async (data) => {
		console.log(data);
		const url = `https://project-comsol.herokuapp.com/api/video/${id}/create`;

		const { videoURL, detail, title } = data;
		...
		await axios.post(url, { videoURL, detail, title }, config);
					...
					<Holder>
						<HolderCard>
							...
							<Form onSubmit={onSubmit} type="multipart/form-data">
								...
								<Label>Please insert Youtube URL for your Pitch </Label>
								<LabelError>
									{errors.videoURL && errors?.videoURL.message}
								</LabelError>
								<Input
									placeholder="Please insert Youtube URL for your Pitch"
									{...register("videoURL")}
								/>
