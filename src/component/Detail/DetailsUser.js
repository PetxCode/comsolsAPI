...
const DetailsUser = () => {
	...
	const getEvent = async () => {
		...
		if (res) {
			...
			console.log("journey data: ", event);
		}
	};

	const getJorney = async () => {
		const url = `https://project-comsol.herokuapp.com/api/journey/${id}`;
		...
		<Container>
			<Left>
				...
				<Details>
					...
					<Detail1>
						<span>
							<label onClick={displayInput3}>{/* <AiOutlineEdit /> */}</label>
						</span>
			...
			<Right>
				...
				<Youtube
					src={`https://www.youtube.com/embed/${
						video?.video[0]?.videoURL.split("=")[0]
					}`}
